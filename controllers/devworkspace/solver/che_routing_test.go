...
func TestDefaultCPULimitIfLimitRangeExists(t *testing.T) {
	cheCluster := &chev2.CheCluster{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "eclipse-che",
			Name:      "eclipse-che",
	...
	err := cheSolver.provisionPodAdditions(objs, cheCluster, routing)
	assert.NoError(t, err)
	assert.Equal(t, constants.DefaultGatewayCpuLimit, objs.PodAdditions.Containers[0].Resources.Limits.Cpu().String())
}
