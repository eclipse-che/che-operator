...
RUN microdnf install -y dnf && dnf -y -q install httpd-tools && dnf -y -q update && dnf -y -q clean all && rm -rf /var/cache/yum && echo "Installed Packages" && rpm -qa | sort -V && echo "End Of Installed Packages"
CMD ["che-operator"]
