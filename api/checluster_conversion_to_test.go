...
func TestConvertTo(t *testing.T) {
	...
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.ClusterRoles, []string{"CheClusterRoles_1", "CheClusterRoles_2"})
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.ExtraProperties, map[string]string{"a": "b", "c": "d"})
	assert.True(t, checlusterv2.Spec.Operands.CheServer.Debug)
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].Image, "CheImage:CheImageTag")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].Name, defaults.GetCheFlavor())
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].ImagePullPolicy, corev1.PullPolicy("Always"))
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].Resources.Limits.Cpu, "ServerCpuLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].Resources.Limits.Memory, "ServerMemoryLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].Resources.Requests.Cpu, "ServerCpuRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Deployment.Containers[0].Resources.Requests.Memory, "ServerMemoryRequest")
	assert.Equal(t, *checlusterv2.Spec.Operands.CheServer.Deployment.SecurityContext.FsGroup, int64(64))
	assert.Equal(t, *checlusterv2.Spec.Operands.CheServer.Deployment.SecurityContext.RunAsUser, int64(65))
	assert.Equal(t, checlusterv2.Spec.Workspaces.TrustedCerts.GitTrustedCertsConfigMapName, "che-git-self-signed-cert")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.LogLevel, "CheLogLevel")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Proxy.CredentialsSecretName, "ProxySecret")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Proxy.NonProxyHosts, []string{"NonProxyHosts_1", "NonProxyHosts_2"})
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Proxy.Port, "ProxyPort")
	assert.Equal(t, checlusterv2.Spec.Operands.CheServer.Proxy.Url, "ProxyURL")
	...
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].Name, defaults.GetCheFlavor()+"-dashboard")
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].Image, "DashboardImage")
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].ImagePullPolicy, corev1.PullPolicy("Always"))
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].Resources.Limits.Cpu, "DashboardCpuLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].Resources.Limits.Memory, "DashboardMemoryLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].Resources.Requests.Cpu, "DashboardCpuRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.Deployment.Containers[0].Resources.Requests.Memory, "DashboardMemoryRequest")
	assert.Equal(t, *checlusterv2.Spec.Operands.Dashboard.Deployment.SecurityContext.FsGroup, int64(64))
	assert.Equal(t, *checlusterv2.Spec.Operands.Dashboard.Deployment.SecurityContext.RunAsUser, int64(65))
	assert.Equal(t, checlusterv2.Spec.Operands.Dashboard.HeaderMessage.Text, "DashboardWarning")
	assert.True(t, checlusterv2.Spec.Operands.Dashboard.HeaderMessage.Show)

	assert.Equal(t, checlusterv2.Spec.Operands.Database.CredentialsSecretName, "ChePostgresSecret")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].Name, constants.PostgresName)
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].Image, "PostgresImage")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].ImagePullPolicy, corev1.PullPolicy("Always"))
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].Resources.Limits.Cpu, "DatabaseCpuLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].Resources.Limits.Memory, "DatabaseMemoryLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].Resources.Requests.Cpu, "DatabaseCpuRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Deployment.Containers[0].Resources.Requests.Memory, "DatabaseMemoryRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.ExternalDb, true)
	assert.Equal(t, checlusterv2.Spec.Operands.Database.PostgresDb, "ChePostgresDb")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.PostgresHostName, "ChePostgresHostName")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.PostgresPort, "ChePostgresPort")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Pvc.ClaimSize, "DatabasePvcClaimSize")
	assert.Equal(t, checlusterv2.Spec.Operands.Database.Pvc.StorageClass, "PostgresPVCStorageClassName")

	assert.Equal(t, checlusterv2.Spec.Operands.DevWorkspace.Deployment.Containers[0].Image, "ControllerImage")
	assert.Equal(t, checlusterv2.Spec.Operands.DevWorkspace.RunningLimit, "RunningLimit")

	assert.Equal(t, checlusterv2.Spec.Operands.ImagePuller.Enable, true)
	assert.Equal(t, checlusterv2.Spec.Operands.Metrics.Enable, true)

	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].Name, constants.DevfileRegistryName)
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].Image, "DevfileRegistryImage")
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].ImagePullPolicy, corev1.PullPolicy("Always"))
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].Resources.Limits.Cpu, "DevfileRegistryCpuLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].Resources.Limits.Memory, "DevfileRegistryMemoryLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].Resources.Requests.Cpu, "DevfileRegistryCpuRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.Deployment.Containers[0].Resources.Requests.Memory, "DevfileRegistryMemoryRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.DisableInternalRegistry, true)
	assert.Equal(t, checlusterv2.Spec.Operands.DevfileRegistry.ExternalDevfileRegistries, []chev2.ExternalDevfileRegistry{
		{
			Url: "ExternalDevfileRegistries_1",
		},
		{
	...
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].Name, constants.PluginRegistryName)
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].Image, "PluginRegistryImage")
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].ImagePullPolicy, corev1.PullPolicy("Always"))
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].Resources.Limits.Cpu, "PluginRegistryCpuLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].Resources.Limits.Memory, "PluginRegistryMemoryLimit")
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].Resources.Requests.Cpu, "PluginRegistryCpuRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.Deployment.Containers[0].Resources.Requests.Memory, "PluginRegistryMemoryRequest")
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.DisableInternalRegistry, true)
	assert.Equal(t, checlusterv2.Spec.Operands.PluginRegistry.ExternalPluginRegistries, []chev2.ExternalPluginRegistry{{Url: "PluginRegistryUrl"}})
