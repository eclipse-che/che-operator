...
- When reading objects, use `ClientWrapper.GetIgnoreNotFound` — do **not** use `Client.Get`. `GetIgnoreNotFound` returns `(found bool, err error)` instead of returning a "not found" error.
