...
func addPartOfCheLabelToUserDefinedObjects(ctx *chetypes.DeployContext) error {
	var objects2Migrate []Object2Migrate

	if infrastructure.IsServiceMonitorEnabled() {
		objects2Migrate = append(objects2Migrate,
			Object2Migrate{
				metrics.CheServerServiceMonitorName,
				&monitoringv1.ServiceMonitor{},
			})

		objects2Migrate = append(objects2Migrate,
			Object2Migrate{
				metrics.DWOServiceMonitorName,
				&monitoringv1.ServiceMonitor{},
			})
	}

	tlsSecretName := utils.GetValue(ctx.CheCluster.Spec.Networking.TlsSecretName, constants.DefaultCheTLSSecretName)
	objects2Migrate = append(
		objects2Migrate,
		Object2Migrate{
			tlsSecretName,
			&corev1.Secret{},
	...
	for _, obj := range objects2Migrate {
		if err := addPartOfCheLabelToObject(
			ctx,
			obj.name,
			obj.objectMeta,
		); err != nil {
...
func addPartOfCheLabelToObject(ctx *chetypes.DeployContext, name string, obj client.Object) error {
	...
	if setPartOfLabel(obj) {
		if err := ctx.ClusterAPI.NonCachingClient.Update(context.TODO(), obj); err != nil {
			return err
		}
	}

	log.Info(getObjectMigratedMessage(obj))

	return nil
}

func setPartOfLabel(obj client.Object) bool {
	labels := utils.GetMapOrDefault(obj.GetLabels(), map[string]string{})

	if labels[constants.KubernetesPartOfLabelKey] == constants.CheEclipseOrg {
		return false
	}
