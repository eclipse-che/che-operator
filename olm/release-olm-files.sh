...
do
  ...
  LAST_NIGHTLY_CSV="${ROOT_PROJECT_DIR}/deploy/olm-catalog/che-operator/eclipse-che-preview-${platform}/manifests/che-operator.clusterserviceversion.yaml"
  LAST_NIGHTLY_CRD="${ROOT_PROJECT_DIR}/deploy/olm-catalog/che-operator/eclipse-che-preview-${platform}/manifests/org_v1_che_crd.yaml"
  ...
  echo "[INFO] Last nightly version: ${lastPackageNightlyVersion}"
  ...
  echo "[INFO] Will create release '${RELEASE}' from nightly version that will replace previous release '${lastPackagePreReleaseVersion}'"
